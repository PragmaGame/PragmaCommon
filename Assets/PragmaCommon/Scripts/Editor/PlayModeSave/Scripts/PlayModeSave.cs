...
namespace PragmaCommon.Editor.PlayModeSave
{
    public class PlayModeSave : EditorWindow
    {
        ...
        private static bool ValidateAlwaysSaveOnExitMenu(MenuCommand command)
           => (EditorSceneManager.IsPreviewScene((command.context as Component).gameObject.scene)
            ...
            : !ComponentsDataBase.Contains(new ComponentSaveDataKey(command.context as Component), out ComponentSaveDataKey foundKey);
        ...
        private static void ApplyMenu(MenuCommand command)
        {
            var key = GetKey(command.context);
            Apply(key);
            if (!ComponentsDataBase.Contains(key, out ComponentSaveDataKey foundKey)) CompDataRemoveKey(key);
        }
        ...
        #region WINDOW
        [MenuItem("Tools/Custom Plugins/" + TOOL_NAME, false, int.MaxValue)]
                ...
                void SaveSelection(SaveCommand cmd, bool always)
                {
                    foreach (var obj in selection)
                    {
                        ...
                        if (cmd == SaveCommand.SAVE_ON_EXITING_PLAY_MODE)
                        {
                            ...
                            if (always) ComponentsDataBase.AlwaysSaveFull(objKey);
                        }
                    }
                }
                    ...
                    if (GUILayout.Button("Remove all components from save list"))
                    {
                        foreach (var obj in selection)
                        {
                           ...
                           : obj.GetComponents<Component>();
                            ...
                            ComponentsDataBase.RemoveFull(objKey);
                            if (_includeChildren)
                            {
        ...
        [Serializable]
        private class ComponentsDataBase
        {
            private const string FILE_NAME = "ComponentsDataBase";
            private const string RELATIVE_PATH = "/PragmaCommon/Scripts/PlayModeSave/Editor/Resources/";
            ...
            public static void Save(bool refreshDataBase = true)
            {
                ...
                if (!File.Exists(fullFilePath))
                {
                    var directories = Directory.GetDirectories(Application.dataPath,
                        "PragmaCommon", SearchOption.AllDirectories);
            ...
            public static bool Load(bool refreshDataBase = true)
            {
                ...
                _instance = JsonUtility.FromJson<ComponentsDataBase>(jsonTextAsset.text);
                _loaded = true;
            ...
            private static ComponentsDataBase _instance = null;
            private static bool _loaded = false;
            private ComponentsDataBase() { }
            private static ComponentsDataBase instance
            {
                get
                {
                    if (_instance == null) _instance = new ComponentsDataBase();
                    return _instance;
                }
            }
        }
        ...
        private class ObjectDataKeyBase : ISerializationCallbackReceiver, IEquatable<ObjectDataKeyBase>
        {
            ...
            public void OnAfterDeserialize()
            {
                ...
                ComponentsDataBase.saveAfterLoading = true;
            }
        }
        ...
        private static void Add(Component component, SaveCommand cmd, bool always, bool serialize)
        {
            ...
            var key = new ComponentSaveDataKey(component);
            if (always) ComponentsDataBase.AlwaysSave(key);
        ...
        private static void Apply(ComponentSaveDataKey key)
        {
            if (CompDataContainsKey(ref key))
            {
                ...
                if (comp == null && obj != null && !_componentsToBeDeleted.Contains(key))
                {
                    ...
                    if (ComponentsDataBase.ContainsFull(objKey, out ObjectDataKey objFoundKey)) ComponentsDataBase.AlwaysSave(key);
                }
                if (comp == null) return;
        ...
        private static void ApplyAll()
        {
            ...
            foreach (var id in comIds)
                if (!ComponentsDataBase.Contains(id, out ComponentSaveDataKey foundKey)) CompDataRemoveKey(id);
            Delete();
        ...
        {
            ComponentsDataBase.Load();
            var alwaysSaveFull = ComponentsDataBase.alwaysSaveFullArray;
            foreach (var item in alwaysSaveFull)
            {
                var obj = FindObject(item);
                if (obj == null)
                {
                    ComponentsDataBase.RemoveFull(item);
                    continue;
                }
                var objKey = new ObjectDataKey(obj);
        ...
        {
            if (!ComponentsDataBase.Load()) return;
            foreach (var key in ComponentsDataBase.alwaysSaveArray)
            {
                GameObject obj = null;
                var comp = FindComponent(key, out obj);
                if (obj == null || comp == null)
                {
                    ...
                    else
                    {
                        ...
                        if (activeScene.path == key.objKey.scenePath) ComponentsDataBase.Remove(key);
                    }
                    continue;
                }
        ...
        {
            if (!ComponentsDataBase.Load(false)) return;
            foreach (var key in ComponentsDataBase.alwaysSaveArray) key.UpdateObjKey();
            EditorApplication.RepaintHierarchyWindow();
        }
                        ...
                        if (FullObjectDataContains(key, out FullObjectData foundItem))
                        {
                            ...
                            if (_includeChildren)
                            {
                                ...
                                foreach (var child in children)
                                {
                                    ...
                                    AddFullObjectData(objKey, true);
                                    ComponentsDataBase.AlwaysSaveFull(objKey);
                                }
                            }
                        }
                    }
